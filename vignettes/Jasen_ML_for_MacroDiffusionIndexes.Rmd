...
Time to set up framework with caret. Some useful references:

1. Tuning random forests on time series data - https://www.r-bloggers.com/2019/11/tuning-random-forest-on-time-series-data/
2. Prequel to above post - https://www.statworx.com/at/blog/time-series-forecasting-with-random-forest/
3. Rob Hyndman the great on time series cross validation - https://robjhyndman.com/hyndsight/tscvexample/
4. Time series CV in R using caret's "timeslice" method - https://rpubs.com/crossxwill/time-series-cv
...
one_hot_ret_SP500 <- as.factor(ret_SP500)
newdata <- cbind(x_lagged, one_hot_ret_SP500)[-(1:16),]
...
newdata <- cbind(x_lagged, one_hot_ret_SP500)
tuneLength.num <- 5
mod.ranger <- train(SP_500.Adjusted ~ .,
                    data = newdata,
                    method = "ranger",
